...
path.exists('config/triggers.json', function(exists) {
  if (exists) {
    triggers = JSON.parse(fs.readFileSync('config/triggers.json', 'utf8'))
  } else {
    fs.writeFileSync('config/triggers.json', '{}', 'utf8')
    triggers = JSON.parse(fs.readFileSync('config/triggers.json', 'utf8'))
  }
})
...
exports.create = function(trigger, callback) {
  ...
  fs.writeFile('config/triggers.json', JSON.stringify(triggers), function(err) {
    if (err) {
    } else {
      callback()
    }
